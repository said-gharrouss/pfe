...
function UpdateStagiaire({id,onUpdateStagiaire}) {
    ...
    useEffect(() => {
        ...
        let stagiaire = stagiaires.filter(stage => stage.id == id);
        if (stagiaire) {
            setValue('name', stagiaire[0].name);
            setValue('email', stagiaire[0].email);
            setValue('password', stagiaire[0].text);
        }
    },[id,setValue])
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const result = await axiosClient.put(`/admin/stagiaires/${id}`, {...data,text});
            const {status} = result;
            if(status === 200){
                onUpdateStagiaire(null);
            }
...
UpdateStagiaire.propTypes = {
    id: PropTypes.number.isRequired,
    onUpdateStagiaire: PropTypes.func.isRequired,
};
