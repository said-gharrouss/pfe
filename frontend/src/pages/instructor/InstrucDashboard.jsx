...
import { INSTRUCTOR_LOGIN } from "../../router/Index";
import VotreStages from "./VotreStages";
import TousLesStages from "./TousLesStages";
...
function InstrucDashboard() {
    ...
    useEffect(()=>{
        if(isAuthenticated === true){
            ...
            userApi.getUser("instructor").then(({data})=>{
                setUser(data);
    ...
    return (
        ...
        <div className=" bg-white shadow-md">
            <nav className="py-[20px] flex justify-between items-center container">
                ...
                <nav className="flex gap-6" >
                    ...
                    onClick={() => handleSelectPage(1)}>Votre Stage(s)</span>

                    <span href="#" className={`cursor-pointer shrink-0 rounded-lg p-2 text-sm font-medium
                    ${selectPage === 2 ? "span_hover_style" : "text-gray-500 hover:bg-gray-50 hover:text-gray-700"}`}
                    onClick={() => handleSelectPage(2)}>Tous les stages</span>
                </nav>
                </div>
        ...
        { selectPage === 1 && <VotreStages/>}
        { selectPage === 2 && <TousLesStages/>}
        </>
    )
}

export default InstrucDashboard
