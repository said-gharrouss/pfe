...
function StagesNonValidée() {
    const [stages,setStages] = useState(window.localStorage.getItem("stages_unvalidated") ? JSON.parse(window.localStorage.getItem("stages_unvalidated")) : []) ;
    ...
    useEffect(()=>{
        ...
        .then(({data}) => {
            data = data.filter(stage => stage.encadrant_pedagogique === user.name && stage.note === null);
            setStages(data);
            window.localStorage.setItem("stages_unvalidated", JSON.stringify(stages));
        }).catch((error) => {
            console.log(error);
        })
    },[stages]);
        ...
        <div className="mt-[30px] pb-[100px] container">
            ...
            <div className="overflow-x-scroll scroll-bar pb-5">
                ...
                <tr>
                    ...
                    <th className="whitespace-nowrap px-4 py-2 font-bold text-gray-900">Etat</th>
                        ...
                        <tr className={`text-center py-[10px] `} key={key}>
                            ...
                            <td className="whitespace-nowrap px-4 py-4 text-gray-700 font-bold">
                                <button className="bg-green-500 text-white px-[5px] py-[2px] rounded-[4px]"
                                 onClick={() => handleValidate(stage.id)}>Valider</button>
                            </td>
                        </tr>
                    ))
                }
            ...
            :
            <div className="text-[30px] text-gray-400 text-center mt-[50px]">Vous n&#39;avez pas de stage, choisir un stage</div>
            }
        </div>
        {
                id !== null && <AddNote id={id} onAddNote={handleValidate}/>
        }
        </>
    )
}

export default StagesNonValidée
